...
zstd_base::zstd_base()
    : cstream_(ZSTD_createCStream()), dstream_(ZSTD_createDStream()),
      in_(new ZSTD_inBuffer), out_(new ZSTD_outBuffer), eof_(0) {}
...
int zstd_base::deflate(int action) {
  ...
  if (action != zstd::run) {
    result = action == zstd::finish ? ZSTD_endStream(s, out)
...
int zstd_base::inflate(int action) {
  ...
  return action == zstd::finish && in->size == 0 && out->pos == 0
             ? zstd::stream_end
             : zstd::okay;
}
...
void zstd_base::do_init(const zstd_params &p, bool compress, zstd::alloc_func,
                        zstd::free_func, void *) {
  ZSTD_inBuffer *in = static_cast<ZSTD_inBuffer *>(in_);
  ZSTD_outBuffer *out = static_cast<ZSTD_outBuffer *>(out_);
